...
const futureStories = [
  ...
  ["US-040", "Story", "Future MIDI En DAW Integratie", "Envelope Release / Soft Note-Off", "In Review", "Should", "DSP Engineer", 5, "Future", "note-off en sustain pedal release stoppen met korte fade-out in plaats van hard afkappen.", "release-time, sustained audio envelope, CLI diagnostics en docs tests falen.", "SustainedVoiceState release frames, --release-time en soft note-off docs/tests.", "US-035, US-039", "In review: default release_time=0.03s; --release-time 0 zet hard stop terug."],
];
...
future.getRange("A5:N26").values = futureStories;
const futureTable = future.tables.add("A4:N26", true, "FutureMidiDawBacklog");
...
future.getRange("A5:N26").format = {
  wrapText: true,
  verticalAlignment: "top",
};
future.getRange("H5:H26").setNumberFormat("0");
future.getRange("A4:N26").format.borders = { preset: "inside", style: "thin", color: "#D9EAD3" };
future.getRange("A4:N26").format.autofitColumns();
future.getRange("A4:N26").format.autofitRows();
...
summary.getRange("B4:B9").formulas = [
  ...
  ["=COUNTA('Future MIDI DAW'!A5:A26)"],
];
summary.getRange("B10").formulas = [["=SUM('Future MIDI DAW'!H5:H26)"]];
